...
output "neuvector-svc-controller-fed-managed" {
  value       = "${data.kubernetes_service.neuvector-svc-controller-fed-managed.status.0.load_balancer.0.ingress[0].ip}"
  description = "NeuVector fed-managed IP"
}

output "neuvector-svc-controller-fed-master" {
  value       = "${data.kubernetes_service.neuvector-svc-controller-fed-master.status.0.load_balancer.0.ingress[0].ip}"
