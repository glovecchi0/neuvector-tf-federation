...
output "neuvector_svc_controller_fed_managed" {
  value       = data.kubernetes_service.neuvector_svc_controller_fed_managed.status.0.load_balancer.0.ingress[0].hostname
  description = "NeuVector fed-managed IP"
}

output "neuvector_svc_controller_fed_master" {
  value       = data.kubernetes_service.neuvector_svc_controller_fed_master.status.0.load_balancer.0.ingress[0].hostname
